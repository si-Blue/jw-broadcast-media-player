...
;(function($) {
  ...
  function onKeyDown(evt) {
    ...
    /* Let Enter and arrow keys go to native form controls (e.g. language dropdown on TV remote) */
    var active = document.activeElement;
    if (active && (active.tagName === 'SELECT' || active.tagName === 'TEXTAREA' ||
        (active.tagName === 'INPUT' && active.type !== 'button' && active.type !== 'submit'))) {
      /* Don't handle Enter – let browser open dropdown / activate control */
      if (evt.keyCode == 13) return;
      /* Don't handle arrows – let dropdown options receive Up/Down */
      if (KEYMAPPING[evt.keyCode]) return;
    }
